...
# ════════════════════════════════════════════════════════
MAX_EDIT_DELAY       = 6.0   # ceiling — never wait longer than this between edits
EASE_DOWN_AFTER      = 20    # consecutive clean edits before easing delay down
EASE_DOWN_STEP       = 0.1   # how much to ease down each time
BACKOFF_MULTIPLIER   = 1.8   # how much to back off on a FloodWait
...
def get_channel_cap(chat_id) -> int:
    """
    Effective per-channel concurrency cap for THIS moment.

    Normally DEFAULT_MAX_WORKERS (2). But a channel whose adaptive delay
    is still above the base floor (i.e. it's recently been flood-waited
    and hasn't earned its way back down via a clean streak yet -- see
    note_edit_success/note_edit_floodwait) gets throttled down to a
    SINGLE concurrent worker instead.

    Why: 2 workers editing the same already-struggling channel at once
    roughly doubles the edit rate Telegram sees on that chat right when
    it's already sitting near/over its limit -- which makes it MORE
    likely to trip another FloodWait, which resets CHANNEL_COOLDOWN and
    keeps the channel "quiet" for even longer. That compounding is what
    was making a heavily-loaded channel go silent for long stretches
    while its (and newly arriving) files sat pending. Dropping to 1
    worker while the channel is proven-unstable gives it real breathing
    room to settle down; once it's had a long enough clean streak that
    its delay eases back to the floor, it automatically gets its second
    worker back.
    """
    if CHANNEL_DELAY[chat_id] > DEFAULT_EDIT_DELAY:
        return 1
...
async def _fetch_from_oldest_window(now: float):
    ...
    async for job in cursor:
        ...
        if CHANNEL_COOLDOWN.get(ch, 0) > now:
            continue
        if CHANNEL_ACTIVE[ch] >= get_channel_cap(ch):
            continue
        CHANNEL_ACTIVE[ch] += 1
...
async def _fetch_fair_across_channels(now: float):
    ...
    eligible = [
        ...
        if CHANNEL_COOLDOWN.get(ch, 0) <= now and CHANNEL_ACTIVE[ch] < get_channel_cap(ch)
    ]
    if not eligible:
        return None
