...
async def set_caption_message(client, query):
    ...
    instr = await query.message.edit_text(
        text=(
            ...
            "📦 <b>Placeholders</b> — copy and paste:\n"
            "┌──────────────────────────────\n"
            "│ <code>{smart_file_name}</code>  — Auto-built smart name\n"
            "│      (title, season, ep, year,\n"
            "│       audio, codec, quality, ext)\n"
            "│\n"
            "│ <code>{file_name}</code>  — Raw file name\n"
            "│ <code>{file_size}</code>  — File size (e.g. 1.2 GB)\n"
            "│ <code>{default_caption}</code>  — Original caption\n"
            "│ <code>{language}</code>  — Audio language(s)\n"
            "│ <code>{year}</code>  — Release year\n"
            "│ <code>{quality}</code>  — Quality (480p/720p/1080p…)\n"
            "│ <code>{resolution}</code>  — Resolution (e.g. 1920x1080)\n"
            "│ <code>{source}</code>  — Source (BluRay/WEB-DL…)\n"
            "│ <code>{codec}</code>  — Video codec (HEVC/x264…)\n"
            "│ <code>{audio_codec}</code>  — Audio codec (DD5.1/AAC…)\n"
            "│ <code>{season}</code>  — Season (e.g. S01)\n"
            "│ <code>{episode}</code>  — Episode (e.g. E02 / Ep.01-09)\n"
            "│ <code>{subtitle}</code>  — Subtitle tag (ESub/HSub…)\n"
            "│ <code>{extension}</code>  — File extension (mkv/mp4…)\n"
            "│ <code>{empty}</code>  — Empty string (blank line)\n"
            "└──────────────────────────────\n\n"
            ...
            "│ <code>&lt;s&gt;Text&lt;/s&gt;</code>  — <s>Strike</s>\n"
            "│ <code>&lt;code&gt;Text&lt;/code&gt;</code>  — <code>Mono</code>\n"
            ...
            "✍️ <b>Example:</b>\n"
            "<code>&lt;b&gt;{smart_file_name}&lt;/b&gt;\n"
            "📦 Size: {file_size}\n"
            "🌐 Lang: {language}\n"
            "🎬 Quality: {quality} | {source}\n"
            "🔤 Subs: {subtitle}&lt;/code&gt;</code>"
        ),
